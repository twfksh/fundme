...
public class AdminProfile extends SecondaryFrame implements ActionListener {
    // attrubutes
        ...
        try {
            ...
            for(int i = 0; i <= totalLines; i++) {
                ...
                if(username.equals(username_)) {
                    this.fullname = Files.readAllLines(Paths.get("src\\data\\admin_info.txt")).get(i-1);
                    this.password = Files.readAllLines(Paths.get("src\\data\\admin_info.txt")).get(i+1);
                    this.email = Files.readAllLines(Paths.get("src\\data\\admin_info.txt")).get(i+2);
                    this.sques = Files.readAllLines(Paths.get("src\\data\\admin_info.txt")).get(i+3);
                    this.sans = Files.readAllLines(Paths.get("src\\data\\admin_info.txt")).get(i+4);
                    this.regDate = Files.readAllLines(Paths.get("src\\data\\admin_info.txt")).get(i+5);
                    break;
                }
            }
